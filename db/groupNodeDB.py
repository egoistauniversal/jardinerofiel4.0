...
class GroupNodeDB(QtCore.QObject):
    ...
    def select_all(self):
        ...
        try:
            ...
            with connection:
                cursor = connection.cursor()
                cursor.execute("SELECT group_node.id, group_node.name FROM group_node "
    ...
    def insert_row(self, group_name):
        _id = -1
        try:
            connection = sqlite3.connect(str(self._dbfile))
            with connection:
    ...
    def modify_row(self, group_id, new_name):
        try:
            connection = sqlite3.connect(str(self._dbfile))
            with connection:
                cursor = connection.cursor()
                cursor.execute("UPDATE group_node SET name=? WHERE group_node.id=? AND group_node.enabled=1",
                               (new_name, group_id))
        except sqlite3.Error, e:
